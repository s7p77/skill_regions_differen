...
class SkillActorCritic(common.Module):
  ...
  def check_and_clean_data(self,tensor):
    # 检查NaN和无穷大值
    mask = ~torch.isfinite(tensor)
    if mask.any():
      print("产生了无效数字")
      # 如果有无效值，则将它们替换为一个合理的数值，例如0，或者你可以选择其他处理方式
      tensor[mask] = 0
    return tensor
  ...
  def update(self, world_model, start, is_terminal, reward_fn):
    ...
    with common.RequiresGrad(self.critic):
      with torch.cuda.amp.autocast(enabled=self._use_amp):
        ...
        # seq = self.check_and_clean_data(seq['weight'])
        # target = self.check_and_clean_data(target)
