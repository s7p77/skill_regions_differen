...
class ColorWrapper:
    def __init__(self, env, color_scale=1.0):
        self._env = env
        self.color_scale = color_scale

    def reset(self):
        time_step = self._env.reset()
        return self._add_color_noise(time_step)

    def step(self, action):
        time_step = self._env.step(action)
        return self._add_color_noise(time_step)

    def _add_color_noise(self, time_step):
        if 'pixels' in time_step.observation:
            pixels = time_step.observation['pixels']
            noise = np.random.uniform(-25, 25, pixels.shape)  # Add random color noise
            noisy_frame = pixels + noise
            noisy_frame = np.clip(noisy_frame, 0, 255).astype(np.uint8)
            time_step = time_step._replace(observation={'pixels': noisy_frame})
        return time_step
...
def _make_jaco(obs_type, domain, task, frame_stack, action_repeat, seed, img_size,color_scale=1.0):
    ...
    # if obs_type == 'pixels':
        # env = ColorWrapper(env, color_scale=color_scale)  # Add ColorWrapper here
    return env


def _make_dmc(obs_type, domain, task, frame_stack, action_repeat, seed, img_size,color_scale=1.0):
...
def make(name, obs_type, frame_stack, action_repeat, seed, img_size=84, color_scale=1.0):
    ...
    env = make_fn(obs_type, domain, task, frame_stack, action_repeat, seed, img_size, color_scale=color_scale)
